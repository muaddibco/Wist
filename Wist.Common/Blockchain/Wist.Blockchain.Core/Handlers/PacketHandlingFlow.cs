...
using Wist.Blockchain.Core.PerformanceCounters;
using Wist.Core;
...
namespace Wist.Blockchain.Core.Handlers
{
    public class PacketHandlingFlow
    {
            ...
            IBlocksHandlersRegistry blocksProcessorFactory, IPerformanceCountersRepository performanceCountersRepository, ILoggerService loggerService)
        {
            ...
            _parseBlock = new TransformBlock<byte[], PacketBase>((Func<byte[], PacketBase>)ParseMessagePacket, new ExecutionDataflowBlockOptions() { MaxDegreeOfParallelism = 1, BoundedCapacity = 1000000 });
            _processBlock = new ActionBlock<PacketBase>((Action<PacketBase>)DispatchBlock, new ExecutionDataflowBlockOptions() { MaxDegreeOfParallelism = 1, BoundedCapacity = 1000000 });
        ...
        private bool ValidateBlock(PacketBase block)
        {
            ...
            try
            {
                ...
                if (!(block.PacketType == (ushort)PacketType.Registry && block.BlockType == BlockTypes.Registry_RegisterUtxoConfidential))
                {
        ...
        private void DispatchBlock(PacketBase block)
        {
            if (block != null)
            {
                ...
                try
                {
                    ...
                    IEnumerable<IBlocksHandler> blocksProcessors = _blocksHandlersRegistry.GetBulkInstances((PacketType)block.PacketType);
